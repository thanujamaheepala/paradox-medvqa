...
def _get_image_blob(im):
    ...
    # im = im[:, :, ::-1]
    # RGB:
    vg_mean = np.array([0.4593, 0.4364, 0.4087])
    vg_std = np.array([0.2762, 0.2739, 0.2867])

    pvqa_moco_mean = np.array([0.84235808, 0.45357143, 1.12311111])
    pvqa_moco_std = np.array([1.4209607, 1.359375, 1.42977778])

    # RGB:
    pvqa_mean = np.array([0.6779, 0.5576, 0.6587])
    pvqa_std = np.array([0.3254, 0.3045, 0.3217])

    bccd_mean = np.array([0.736, 0.695, 0.714])
    bccd_std = np.array([0.061, 0.102, 0.076])

    im = (im - pvqa_mean) / pvqa_std
    im = im * vg_std + vg_mean

    im -= (cfg.PIXEL_MEANS / 255.0)
...
def get_detections_from_im(fasterRCNN, classes, im_file, image_id, args, conf_thresh=0.001):
    ...
    with torch.no_grad():
        ...
        # print('im_data.std=',im_data.float().std())
    ...
    if len(keep_boxes.shape) == 0:
        keep_boxes = torch.Tensor([keep_boxes])
    ...
    print('pooled_feat std:', pooled_feat[keep_boxes].std())
    return {
        'image_id': image_id,
...
def get_image_ids(image_dir: str):
    files = sorted(os.listdir(image_dir))
    image_ids = [[os.path.join(image_dir, files[i]), i] for i in range(len(files))]
    return image_ids
