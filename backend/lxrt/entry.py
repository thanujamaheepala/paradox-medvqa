...
class LXRTEncoder(nn.Module):
    ...
    def forward(self, sents, feats, target_answers, visual_attention_mask=None, t='vqa'):
        ...
        input_ids = torch.tensor(
            [f.input_ids for f in train_features], dtype=torch.long).cpu()
        input_mask = torch.tensor(
            [f.input_mask for f in train_features], dtype=torch.long).cpu()
        segment_ids = torch.tensor(
            [f.segment_ids for f in train_features], dtype=torch.long).cpu()
